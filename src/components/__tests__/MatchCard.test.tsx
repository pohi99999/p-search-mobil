...
describe('MatchCard', () => {
  ...
  it('renders correctly with full grant data', () => {
    ...
    // Test amount
    const amountInstances = root!.root.findAll((node) => {
      ...
      let childrenStr = Array.isArray(node.props.children)
  ...
  it('renders correctly with missing optional fields', () => {
    ...
    const amountInstances = root!.root.findAll((node) => {
      if (node.type !== 'Text' || !node.props.children) return false;
      const childrenStr = Array.isArray(node.props.children)
        ? node.props.children.join('')
        : String(node.props.children);
  ...
  it('calls onPress when the button is pressed', () => {
    ...
    const btn = root!.root.find((node) => {
      return (
        node.type === require('react-native-paper').Button || node.props.testID === 'details-button'
      );
    });
