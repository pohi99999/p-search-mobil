...
import { Card, Text, ProgressBar, Divider, List, Button } from 'react-native-paper';
...
export function ActionPlanCard({ plan, planTasks, stats, profile, onStatusChange, refetch }: Props) {
  const [pdfLoading, setPdfLoading] = useState(false);
  const { showAdIfAvailable } = useInterstitialAd();
  const { totalTasks, completedTasks, progress, percentage } = stats;
        ...
        <List.Section style={styles.listSection}>
          {planTasks.map((task, index) => (
            <React.Fragment key={task.id}>
              <TaskItem task={task} onStatusChange={onStatusChange} />
