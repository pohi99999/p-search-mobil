...
export function ActionPlanScreen({ route, navigation }: ActionPlanScreenProps) {
  ...
  const handleStatusChange = useCallback(async (task: ActionTask, currentStatus: ActionTaskStatus) => {
    ...
    try {
      ...
      Alert.alert('Nem sikerült frissíteni a feladat állapotát.');
    }
  }, [updateTaskStatus]);
                    ...
                    <Button
                      ...
                      onPress={async () => {
                        try {
                          ...
                          Alert.alert('Váratlan hiba történt a PDF megnyitásakor. Kérjük, próbálja újra később.');
                        }
                      }}
                  ...
                  <Button 
                    ...
                    onPress={() => {
                      if (!profile || !plan.match_id) {
                        Alert.alert('Nem generálható dokumentum: hiányzó cégprofil vagy pályázati azonosító.');
                        return;
                      }
                      showAdIfAvailable(async () => {
                        ...
                        try {
                          ...
                          Alert.alert('Váratlan hiba történt a PDF generálásakor. Kérjük, próbálja újra később.');
                        } finally {
                          setPdfLoading(false);
                        }
                      });
